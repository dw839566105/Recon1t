...
def Calib(theta, *args):
    EventID, ChannelID, flight_time, PMT_pos, cut, LegendreCoeff = args
    y = flight_time
    T_i = np.dot(LegendreCoeff, theta[0:-1])
    # quantile regression
    # quantile = 0.01
    L0 = Likelihood_quantile(y, T_i, 0.1, theta[-1], EventID)
    L = L0 + np.sum(np.abs(theta))
    return L0

def Likelihood_quantile(y, T_i, tau, ts, EventID):
    #less = T_i[y<T_i] - y[y<T_i]
    #more = y[y>=T_i] - T_i[y>=T_i]
    #R = (1-tau)*np.sum(less) + tau*np.sum(more)
    R = (1-tau)*(T_i-y)*(y<T_i) + (tau)*(y-T_i)*(y>=T_i)
    H,edges =np.histogram(EventID, weights = R, bins = np.hstack((np.unique(EventID), np.max(EventID)+1)))
    Q = np.bincount(EventID)
    #L0 = 0
    L = Q[1:]*np.log(tau*(1-tau)/ts) - H/ts
    L0 = np.nansum(L)
    return - L0
...
def main_Calib(radius, path, fout, cut_max, PMT_pos):
    ...
    with h5py.File(fout,'w') as out:
        ...
        for cut in np.arange(5,cut_max,1):
            theta0 = np.zeros(cut+1) # initial value
            ...
            result = minimize(Calib, theta0, method='SLSQP',\
                    bounds=((None, None),(None,None), (None,None), (None,None),(None,None),(0.01,100)), \
                    args = (EventID, ChannelID, PETime, PMT_pos_rep, cut, LegendreCoeff[:,0:cut]))
            print(theta0[-1],result.fun)
            theta0[-1] = 2
            args = (EventID, ChannelID, PETime, PMT_pos_rep, cut, LegendreCoeff[:,0:cut])
            result = minimize(Calib, theta0, method='SLSQP',\
                    bounds=((None, None),(None,None), (None,None), (None,None),(None,None),(0.01,100)), \
