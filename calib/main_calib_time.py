...
def Calib(theta, *args):
    Channel_tmp, qt, PMT_pos, cut = args
    y = qt
    # fixed axis
    x = Legendre_coeff(PMT_pos[Channel_tmp,:])
    #print(x.shape)
    #print(theta.shape)
    #print(qt, qt.shape)
    #exit()
    # print(PMT_pos[ChannelID,:])
    # print(x.shape, PETime.shape)
    # exit()
    # Poisson regression
    # L = - np.sum(np.sum(np.transpose(y)*np.transpose(np.dot(x, theta)) \
    #    - np.transpose(np.exp(np.dot(x, theta)))))
    L = np.log(np.sum((np.transpose(np.dot(x, theta))-y)**2))
    # print(L)
    return L
...
def main_Calib(Energy, radius, fout):
    ...
    Channel_tmp = np.arange(np.max(ChannelID))
    qt = np.zeros_like(Channel_tmp,dtype='float')
    for k in Channel_tmp:
        '''
        time_offset = np.mean(PETime[EventID==k])
        #print(time_offset)
        PETime[EventID==k] = PETime[EventID==k] - time_offset
        #print(PETime[EventID==k])
        ...
        '''
        qt[k] = np.quantile(flight_time[(ChannelID==k) & (flight_time!=0)],0.03)
                                        
    theta0 = np.ones(cut) # initial value
    result = minimize(Calib,theta0, method='SLSQP',args = (Channel_tmp, qt, PMT_pos, cut))  
