...
def main_Calib(radius, path, fout, cut_max, PMT_pos):
    ...
    with h5py.File(fout,'w') as out:
        ...
        tmp = time.time()
        EventNo = np.size(np.unique(EventIDx))
        PMTNo = np.size(PMT_pos[:,0])
        #vertex = np.tile(vertex,(PMTNo,1))
        #PMT_pos_rep = np.repeat(PMT_pos, EventNo, axis=0)
       ...
       ''' 
        ...
        for cut in np.arange(2,cut_max,1): # just take special values
            ...
            print('%d-th coeff:\n' % cut, reg.coef_,'\n')
            args = y, PMT_pos_rep, cut, LegendreCoeff[:,0:cut]
            L = Calib(reg.coef_, *args) + 2*cut
            print(L)
