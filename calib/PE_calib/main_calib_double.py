...
def main_Calib(radius, path, fout, cut_max):
    ...
    with h5py.File(fout,'w') as out:
        ...
        # positive direction
        EventIDx1, ChannelIDx1, xx1, yx1, zx1 = readchain('+' + radius, path, 'y')
        EventIDx2, ChannelIDx2, xx2, yx2, zx2 = readchain('-' + radius, path, 'y')
        x1 = np.array((xx1[0], yx1[0], zx1[0]))
        x2 = np.array((xx2[0], yx2[0], zx2[0]))
        size1 = np.size(np.unique(EventIDx1))
        size2 = np.size(np.unique(EventIDx2))
        
        EventIDx2 = EventIDx2 + np.max(EventIDx1)
        EventID = np.hstack((EventIDx1, EventIDx2))
        ChannelID = np.hstack((ChannelIDx1, ChannelIDx2))
        size = size1 + size2
        ...
        tmp_x_p = np.tile(tmp_x_p, (size1,1))           
        tmp_x_n = Legendre_coeff(PMT_pos, x2/1e3, cut_max)
        tmp_x_n = np.tile(tmp_x_n, (size2,1))
        LegendreCoeff = np.vstack((tmp_x_p, tmp_x_n))
       ...
       ''' 
        ...
        for cut in np.arange(1,cut_max,1): # just take special values
            ...
            vs_x = np.reshape(total_pe[0:size*30],(-1,30), order='C')
            ...
            #aaa = np.reshape(L0[0:size*30],(30,-1),order='F')
            ...
            out.create_dataset('AIC' + str(cut), data = aic1)
