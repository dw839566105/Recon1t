...
def main_Calib(path, fout, cut_max, PMT_pos):
    '''
    # main program
    ...
    with h5py.File(fout,'w') as out:
        ...
        # positive direction
        EventID, Q, xx, yx, zx = readchain(path,'+')
        ...
        rho = np.linalg.norm(vertex, axis=1)/1000/0.65
        theta = np.arccos(cos_theta)

        cart = RZern(10)
        nk = cart.nk
        
        basis = np.zeros((rho.shape[0], nk))

        for i in np.arange(nk):
            if not i % 5:
                print(f'process {i}-th event')
            basis[:,i] = cart.Zk(i, rho, theta)
        print(f'use {time.time() - tmp} s')
        print(np.sum(np.isnan(basis)))
        print(rho.shape)
        ...
        out.create_dataset('std', data = std)
        out.create_dataset('AIC', data = L)

if len(sys.argv)!=4:
    print("Wront arguments!")
    print("Usage: python main_calib.py 'path' outputFileName[.h5] Max_order")
    sys.exit(1)
