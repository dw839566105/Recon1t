...
def Likelihood(vertex, *args):
    ...
    L1, expect = Likelihood_PE(vertex, *(coeff_pe, PMT_pos, pe_array, cut_pe, str_s))
    L2, T_i = Likelihood_Time(vertex, *(coeff_time, PMT_pos, fired_PMT, time_array, cut_time, str_s))
    return L1+L2
...
def recon(fid, fout, *args):
    ...
    class TmpData(tables.IsDescription):
        EventID = tables.Int64Col(pos=0)    # EventNo
        truth = tables.Float16Col(pos=1)
        inner = tables.Float16Col(pos=2)
        outer = tables.Float16Col(pos=3)
        pin = tables.Float16Col(pos=4)
        pout = tables.Float64Col(pos=5)
    # Create the output file and the group
    ...
    TmpTable = h5file.create_table(group, "Tmp", TmpData, "Tmp")
    tmpdata = TmpTable.row
    # Loop for event
    ...
    for chl, Pkl, xt, yt, zt, Et in zip(a.array("PEList.PMTId"),
        ...
        tmp1, expect_pe_in = Likelihood_PE(result_in.x, *(coeff_pe_in, PMT_pos, pe_array, cut_pe, 'in'))

        tmp2, expect_time = Likelihood_Time(result_in.x, *(coeff_time_in, PMT_pos, fired_PMT, time_array, cut_time, 'in'))
        #print(expect_time.T)
        # outer recon
        # initial value
        x0_out = x0_in.copy()
        ...
        tmp1, expect_pe_out = Likelihood_PE(result_out.x, *(coeff_pe_out, PMT_pos, pe_array, cut_pe, 'out'))
        tmp2, expect_time = Likelihood_Time(result_out.x, *(coeff_time_out, PMT_pos, fired_PMT, time_array, cut_time, 'out'))

        aa = np.exp(-expect_pe_in)*expect_pe_in**pe_array/scipy.special.factorial(pe_array)
        bb = np.exp(-expect_pe_out)*expect_pe_out**pe_array/scipy.special.factorial(pe_array)

        for k in np.arange(np.size(aa)):
            tmpdata['EventID'] = event_count
            tmpdata['inner'] = expect_pe_in[k]
            tmpdata['outer'] = expect_pe_out[k]
            tmpdata['pin'] = aa[k]
            tmpdata['pout'] = bb[k]
            tmpdata['truth'] = pe_array[k]
            tmpdata.append()
        recondata.append()
        print('inner')
        print(result_in.fun)
