...
def recon(fid, fout, *args):
    ...
    class ReconData(tables.IsDescription):
        ...
        # MC Sph
        E_sph_MC = tables.Float16Col(pos=24)        # Energy
        x_sph_MC = tables.Float16Col(pos=25)        # x position
        y_sph_MC = tables.Float16Col(pos=26)        # y position
        z_sph_MC = tables.Float16Col(pos=27)        # z position
        t0_MC = tables.Float16Col(pos=28)       # time offset
        success_MC = tables.Int64Col(pos=29)    # recon failure 
        Likelihood_MC = tables.Float16Col(pos=30)
        L_MC = tables.Float16Col(pos=31)
    ...
    for chl, Pkl, xt, yt, zt, Et in zip(a.array("PEList.PMTId"),
        ...
        #print(real_sum, np.sum(pe_array))
        
        # MC
        result_MC0 = Likelihood(np.array((np.log(E_MC/2), bins[index,0]**(1/3)/0.65,np.arccos(bins[index,1]),bins[index,2],0)), *(coeff_time_in, coeff_pe_in, PMT_pos, fired_PMT, time_array, pe_array, cut_time, cut_pe, 'out'))
        
        #print(np.array(bins[index,0]**(1/3)/shell, np.arccos(bins[index,1]), bins[index,2]))
        MC1 = r2c(np.array((bins[index,0]**(1/3)/shell, np.arccos(bins[index,1]), bins[index,2])))*shell
        ...
        if(bins[index,0]**(1/3)>0.5):
            x0 = np.array((np.log(E_MC/2), bins[index,0]**(1/3)/0.65,np.arccos(bins[index,1]),bins[index,2],0), x0_out[0][4])
            result_MC = minimize(Likelihood, x0, method='SLSQP',bounds=((E_min, E_max), (shell_out,1), (None, None), (None, None),(None, None)), args = (coeff_time_out, coeff_pe_out, PMT_pos, fired_PMT, time_array, pe_array, cut_time, cut_pe, 'out'))
        else:
            x0 = np.array((np.log(E_MC/2), bins[index,0]**(1/3)/0.65,np.arccos(bins[index,1]),bins[index,2],0), x0_in[0][4])
            result_MC = minimize(Likelihood, x0, method='SLSQP',bounds=((E_min, E_max), (0, shell_in+1e-6), (None, None), (None, None),(None, None)), args = (coeff_time_in, coeff_pe_in, PMT_pos, fired_PMT, time_array, pe_array, cut_time, cut_pe, 'in'))
        
        MC2 = r2c(result_MC.x[1:4])*shell
        if(result_MC.fun>result_MC0):
            recondata['x_sph_MC'] = MC1[0]
            recondata['y_sph_MC'] = MC1[1]
            recondata['z_sph_MC'] = MC1[2]
            recondata['E_sph_MC'] = result_MC.x[0]
            recondata['success_MC'] = 0
            recondata['Likelihood_MC'] = result_MC0 
        else:
            recondata['x_sph_MC'] = MC2[0]
            recondata['y_sph_MC'] = MC2[1]
            recondata['z_sph_MC'] = MC2[2]
            recondata['E_sph_MC'] = result_MC.x[0]
            recondata['success_MC'] = result_MC.success
        ...
        print('MC')
        print('%d: [%+.2f, %+.2f, %+.2f] radius: %+.2f, Likelihood: %+.2f' % (event_count, MC2[0], MC2[1], MC2[2], norm(MC2), result_MC.fun))
        
        print('MC_raw')
        print('%d: [%+.2f, %+.2f, %+.2f] radius: %+.2f, Likelihood: %+.2f' % (event_count, MC1[0], MC1[1], MC1[2], norm(MC1), -np.max(L_MC)))
        #print(event_count, result_out.x[1:4] * shell, np.sqrt(np.sum(result_out.x[1:4]**2)), result_out.fun)
        print('-'*60)
