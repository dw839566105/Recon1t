...
def recon(fid, fout, *args):
    ...
    for EventNo, chl, Pkl, xt, yt, zt, Et in zip(a.array("TriggerNo"),
        ...
        if np.sum(pe_array)!=0:
            ...
            result_in = dual_annealing(Likelihood,
                    x0 = x0_in[0], 
                    bounds = ((E_min, E_max), (-1, 1), (0, np.pi), (0, np.pi*2), (0, 1024)), 
                    initial_temp = 2000, 
                    args = (coeff_time, coeff_pe, PMT_pos, fired_PMT, time_array, pe_array, cut_time, cut_pe))
