...
def Likelihood_PE(vertex, *args):
    ...
    for i in np.arange(cut):
        if(z>=0.9):
            k[i] = LG.legval(z, coeff_pe_out[i])
        else:
            k[i] = LG.legval(z, coeff_pe_in[i])
    print(k)
    print(z,LG.legval(z, coeff_pe_in.T))
    exit()
    '''
    if(z>=bd):
