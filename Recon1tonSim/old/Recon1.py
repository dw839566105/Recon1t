...
h1 = tables.open_file('../calib/PE_coeff_1t_seg.h5','r')
h2 = tables.open_file('../calib/Time_coeff_1t_seg.h5','r')

def load_coeff(order):
    h = tables.open_file('../calib/PE_coeff_1t' + '15' + '.h5','r')
    coeff_pe_in = h.root.poly_in[:]
    coeff_pe_out = h.root.poly_out[:]
    h.close()
    cut_pe, fitcut_pe = coeff_pe_in.shape

    h = tables.open_file('../calib/Time_coeff_1t' + '15' + '.h5','r')
...
def EC(z):
    ...
    for i in np.arange(1):
        rr = eval('h1.root.poly_r_%d[:]' % i)
        coeff = eval('h1.root.poly_coeff_%d[:]' % i)
        if(rr[1] < 0.05):
            rr[1] == 0     
        ii = np.where(z<rr)
        if(ii[0][0]==0):
...
def Likelihood_Time(vertex, *args):
    ...
    for i in np.arange(cut-1):
        rr = eval('h2.root.poly_r_%d[:]' % i)
        coeff = eval('h2.root.poly_coeff_%d[:]' % i)
        if(rr[1] < 0.05):
            rr[1] = 0     
        ii = np.where(z<rr)
        if(np.size(ii)==0):
...
def con_sph_in(args):
    ...
    t0_max\
    = args
    cons = ({'type': 'ineq', 'fun': lambda x: 1 - (x[1]**2 + x[2]**2 + x[3]**2)})
    return cons
...
def recon(fid, fout, *args):
    ...
    for chl, Pkl, xt, yt, zt, Et in zip(a.array("PEList.PMTId"),
        ...
        result_in = minimize(Likelihood, x0, method='SLSQP',bounds=((E_min, E_max), (0, 1), (None, None), (None, None), (None, None)), args = (coeff_time_in, coeff_pe_in, PMT_pos, fired_PMT, time_array, pe_array, cut_time, cut_pe, 'in'))
        ...
        # initial value
        x0_out = x0_in.copy()
        x0_out[0][1] = in2[0]/np.sqrt(np.sum(in2[0]**2+in2[1]**2+in2[2]**2))*0.9
        x0_out[0][2] = in2[1]/np.sqrt(np.sum(in2[0]**2+in2[1]**2+in2[2]**2))*0.9
        x0_out[0][3] = in2[2]/np.sqrt(np.sum(in2[0]**2+in2[1]**2+in2[2]**2))*0.9
        a = c2r(x0_out[0][1:4])
        # not added yet
        con_args = E_min, E_max, tau_min, tau_max, t0_min, t0_max
        cons_sph_out = con_sph_out(con_args)
        x0 = np.hstack((x0_out[0][0], a, x0_out[0][4]))
        result_out1 = minimize(Likelihood, x0, method='SLSQP',bounds=((E_min, E_max), (0,1), (None, None), (None, None),(None, None)), args = (coeff_time_out, coeff_pe_out, PMT_pos, fired_PMT, time_array, pe_array, cut_time, cut_pe, 'out'))
        ...
        cons_sph_out = con_sph_out(con_args)
        x0 = np.hstack((x0_out[0][0], a, x0_out[0][4]))
        result_out2 = minimize(Likelihood, x0, method='SLSQP',bounds=((E_min, E_max), (0,1), (None, None), (None, None),(None, None)), args = (coeff_time_out, coeff_pe_out, PMT_pos, fired_PMT, time_array, pe_array, cut_time, cut_pe, 'out'))
        
        if(result_out1.fun < result_out2.fun):
        ...
        recondata.append()
        print('inner')
        if(~np.isnan(result_in.fun)):
            print('%d: %.2f MeV, [%+.2f, %+.2f, %+.2f] radius: %+.2f, Likelihood: %+.2f' % (event_count, np.exp(result_in.x[0] - EC(norm(in2)) + np.log(2)), in2[0], in2[1], in2[2], norm(in2), result_in.fun))
        else:
            print(result_in.x)
        #print(event_count, result_in.x[1:4] * shell, np.sqrt(np.sum(result_in.x[1:4]**2)),result_in.fun)
        print('outer')
        if(~np.isnan(result_out.fun)):
            print('%d: %.2f MeV, [%+.2f, %+.2f, %+.2f] radius: %+.2f, Likelihood: %+.2f' % (event_count, np.exp(result_out.x[0] - EC(norm(in2)) + np.log(2)), out2[0], out2[1], out2[2], norm(out2), result_out.fun))
        else:
            print(result_out.x)
