...
def Likelihood(vertex, *args):
    ...
    z, x, E = Calc_basis(vertex, PMT_pos, np.max((cut_time, cut_pe)))
    L1 = Likelihood_PE(E, z, x, coeff_pe, pe_array, cut_pe)
    L2 = Likelihood_Time(z, x, vertex[4], coeff_time, fired_PMT, time_array, cut_time)
    return L1+L2

def LikelihoodPE(vertex, *args):
    '''
    vertex[1]: r
    vertex[2]: theta
    vertex[3]: phi
...
def Likelihood_PE(E, z, x, coeff, pe_array, cut):
    # Recover coefficient
...
def recon(fid, fout, *args):
    ...
    for i in np.arange(tp_out.shape[0]):
        vertex[2:4] = mesh[i]
        z, x, E = Calc_basis(vertex, PMT_pos, cut_pe)
        # Recover expect
        expect = np.exp(np.dot(x,k))
        tp_out[i] = expect
    # Gain = 164
    ...
    for EventNo, chl, Pkl, xt, yt, zt, Et in zip(a.array("TriggerNo"),
        ...
        if ((np.sum(pe_array)!=0) & (EventNo == 742)):
            ...
            print(LikelihoodPE(result_in.x, *(coeff_time, coeff_pe, PMT_pos, fired_PMT, time_array, pe_array, cut_time, cut_pe)))

            print(LikelihoodPE(result_out.x, *(coeff_time, coeff_pe, PMT_pos, fired_PMT, time_array, pe_array, cut_time, cut_pe)))
            print('-'*60)
