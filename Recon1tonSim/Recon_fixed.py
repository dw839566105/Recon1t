...
for chl, Pkl, xt, yt, zt, Et in zip(a.array("PEList.PMTId"),
    ...
    if ((np.sum(pe_array)!=0) & (zt>80) & (zt<110)):
        ...
        theta = np.arccos(zt/1000/r)
        phi = np.arctan2(yt, xt)
        EE = legval(r, coeff_pe.T)[0]
        x0_in[0][0] = EE
        x0_in[0][1] = r
        x0_in[0][2] = zt/1000/r
        x0_in[0][3] = phi

        result_in = minimize(Likelihood, x0_in, method='SLSQP',bounds=((E_min, E_max), (0, 1), (None, None), (-np.pi,np.pi), (None, None)), args = (coeff_time, coeff_pe, PMT_pos, fired_PMT, time_array, pe_array, cut_time, cut_pe))
