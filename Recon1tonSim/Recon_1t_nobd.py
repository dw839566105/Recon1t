...
def load_coeff():
    h = tables.open_file('../calib/PE_coeff_1t_29_50.h5','r')
    coeff_pe = h.root.coeff_L[:]
    h.close()
    cut_pe, fitcut_pe = coeff_pe.shape

    h = tables.open_file('../calib/Time_coeff2_1t_0.05.h5','r')
...
def Likelihood_Time(vertex, *args):
    ...
    k = np.zeros(cut)
    k = LG.legval(z, coeff_time.T)
    k[0] = vertex[4]
    T_i = np.dot(x, k)
    L = np.nansum(Likelihood_quantile(y, T_i, 0.1, 2.5))
...
def recon(fid, fout, *args):
    ...
    for chl, Pkl, xt, yt, zt, Et in zip(a.array("PEList.PMTId"),
        ...
        if np.sum(pe_array)!=0:
            ...
            result_out = minimize(Likelihood, x0, method='SLSQP',bounds=((E_min, E_max), (0,1), (None, None), (None, None),(None, None)), args = (coeff_time, coeff_pe, PMT_pos, fired_PMT, time_array, pe_array, cut_time, cut_pe))
