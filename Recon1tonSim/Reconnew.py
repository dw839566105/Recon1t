...
h = tables.open_file('/home/douwei/LegendreCoeff.h5','r')
recondata = h.root.coeff[:]
coeff = recondata[:]
cut = np.size(coeff,0)-1
...
def Likelihood_Sph(vertex, *args):
    ...
    x = np.zeros((size, cut+1))
    for i in np.arange(0,cut+1):
        c = np.zeros(cut+1)
        c[i] = 1
    ...
    for i in np.arange(np.size(coeff,0)):    
        k[i] = np.polynomial.legendre.legval(z,coeff[i,:])
