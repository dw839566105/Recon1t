...
def LoadDataTime(path, radius, order):
    data = []
...
## gather the data
path = './coeff_time_1t_2.0MeV_dns_Lasso/'
...
for radius in ra:
    ...
    k = LoadDataTime(path, str_radius, order)
    rd.append(np.array(radius))
    coeff_time = np.hstack((coeff_time, k))

coeff_time = np.reshape(coeff_time,(-1,np.size(ra)),order='F')
with h5py.File('./Time_coeff_1t_seg.h5','w') as out:
    for i in np.arange(np.size(coeff_time[:,0])):
        ...
        for l in np.arange(1+1):
            ...
            xx = np.hstack((xx,x2))
            if(l == 0):
                poly_r = np.hstack((poly_r, x[r_set[0]]))
