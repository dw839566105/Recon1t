...
def readtpl():
    ...
    h = tables.open_file("./Grid_template.h5")
    tp = h.root.template[:]
...
def load_coeff():
    ...
    h = tables.open_file('./PE_coeff_1t_29_80.h5','r')
    coeff_pe = h.root.coeff_L[:]
    h.close()
    cut_pe, fitcut_pe = coeff_pe.shape

    h = tables.open_file('./Time_coeff_1t_0.1.h5','r')
...
def recon(fid, fout, *args):
    ...
    for Event in EID:
        ...
        if np.sum(pe_array)!=0:
            ...
            #print('------------------->',result_out.x[1], base_out[0])
            print('inner')
...
if len(sys.argv)!=4:
    ...
    print("Usage: python Recon.py WaveformAnalyzedFileName[.h5] outputFileName[.h5] Calibration[.txt]")
    sys.exit(1)
