...
h1 = tables.open_file('../calib/PE_coeff.h5','r')
coeff_pe = h1.root.coeff[:]
h1.close()
cut_pe, fitcut = coeff_pe.shape

h2 = tables.open_file('../calib/Time_coeff.h5','r')
coeff_time = h2.root.coeff[:]
h2.close()

cut_time, fitcut = coeff_time.shape
...
def Likelihood(vertex, *args):
    coeff_pe, coeff_time, PMT_pos, event_pe, fired, time, cut_pe, cut_time = args
    L1 = Likelihood_Sph(vertex, *(coeff_pe, PMT_pos, event_pe, cut_pe))
    L2 = Likelihood_Time(vertex, *(coeff_time, PMT_pos, fired, time, cut_time))
    return L1 + 0.5*L2
...
def Likelihood_Time(vertex, *args):
    ...
    #L = Likelihood_quantile(y, T_i[:,0], 0.05, 26)
    tc = y-T_i[:,0];
    tc[tc<=-8] = -8
    tc[tc>60] = 60
    log_p_time = TimeUncertainty(tc, 26)
    L = - np.nansum(log_p_time)
    return L

'''
def Likelihood_Time(vertex, *args):
    coeff, PMT_pos, fired_PMT, time_array, cut = args
    Energy = vertex[0]
    x = vertex[1]
    y = vertex[2]
    z = vertex[3]
    ...
    distance = SolidAngle(x,y,z)

    # log Time profile pdf
    log_p_time = TimeProfile(time_array, distance[fired_PMT], 26, t)
    L = - np.nansum(log_p_time)
    return L
'''
...
def recon(fid, fout, *args):
    ...
    for chl, Pkl, xt, yt, zt, Et in zip(a.array("PEList.PMTId"),
        ...
        time_array = np.zeros(0, dtype=float)    # Time info (Hit number)
        ...
        # initial result
        result_vertex = np.zeros((0,5)) # reconstructed vertex
        ...
        record = np.zeros((1,4))
        result = minimize(Likelihood, x0, method='SLSQP',constraints=cons_sph, args = (coeff_pe, coeff_time, PMT_pos, pe_array, fired_PMT, time_array, cut_pe, cut_time))
        result1 = minimize(Likelihood_Sph, x0, method='SLSQP',constraints=cons_sph, args = (coeff_pe, PMT_pos, pe_array, cut_pe))
