...
coeff_pe1 = h.root.coeff1[:]
coeff_pe2 = h.root.coeff2[:]
h.close()
cut_pe, fitcut_pe = coeff_pe1.shape
coeff_pe = (coeff_pe1,coeff_pe2)

h = tables.open_file('../calib/Time_coeff_1t.h5','r')
coeff_time1 = h.root.coeff1[:]
coeff_time2 = h.root.coeff2[:]
h.close()
cut_time, fitcut_time = coeff_time1.shape
coeff_time = (coeff_time1,coeff_time2)
...
def Likelihood_PE(vertex, *args):
    ...
    for i in np.arange(cut):
        if(np.abs(z)<=0.75):
            ...
            co = coeff[1]
        k[i] = np.sum(np.polynomial.legendre.legval(z,co[i,:]))
...
def Likelihood_Time(vertex, *args):
    ...
    for i in np.arange(cut):
        if(np.abs(z)<=0.75):
            ...
            co = coeff[1]
        k[0,i] = np.sum(np.polynomial.legendre.legval(z,co[i,:]))
...
def recon(fid, fout, *args):
    ...
    for chl, Pkl, xt, yt, zt, Et in zip(a.array("PEList.PMTId"),
        ...
        time_tmp = np.zeros(np.size(PMT_No))
        fired_tmp = np.zeros(np.size(PMT_No))
        weight = np.zeros(np.size(PMT_No))
        for j,k in enumerate(PMT_No):
            time_tmp[j] = np.min(time_array[fired_PMT==k])
            fired_tmp[j] = k
            weight[j] = np.sum([fired_PMT==k])
            
        time_array = time_tmp
        fired_PMT = fired_tmp
        fired_PMT = fired_PMT.astype(int) 
        
        time_array = time_array[weight>5]
        fired_PMT = fired_PMT[weight>5]
        ...
        result = minimize(Likelihood, x0, method='SLSQP',constraints=cons_sph, args = (coeff_time, coeff_pe, PMT_pos, fired_PMT, time_array, pe_array, cut_time, cut_pe))
